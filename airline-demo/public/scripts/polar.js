...
function PolarChart(map, feature, options) {
    this.options = options || {};
    this.map = map;
    this.feature = feature;
}

PolarChart.prototype.create = function(airport) {
    this.airport = airport;
    this.uid = this.airportCode = airport.airportCode;
    var node = $('#'+this.uid+'_chart')[0];
    this.tooltip = new Tooltip({id:'charttooltip_'+this.airportCode});
   ...
   $('#'+this.airportCode+'_showopts').on('click',function(){
        //console.log('showOpts');
        this.showOptions(this.feature);
    }.bind(this))
    $('#'+this.airportCode+'_hideopts').on('click',function(){
        this.hideOptions(this.feature);
    }.bind(this))

    this.chartOptions = new PolarChartOptions(this);
}
...
function PolarChartOptions(chart) {
    this.chart = chart;
    this.airport = chart.airport;
    this.map = chart.map;
    this.uid = chart.airportCode;
...
PolarChartOptions.prototype.setCarriers = function(carriers) {
    carriers = carriers || [];
    var me = this;
    var carrieropts = [];
    ...
    $('#'+this.uid+'_airline_code').on("selectmenuselect", function(event,ui) {
        ...
        me.clearFlightNumbers();
        me.airport.getScheduleByCarrierForToday(function(data){
            //console.log('Got flightnums for ',ui.item.value,' flightnums: ',data);
            me.setFlightNumbers(data);
        }, ui.item.value);
    });
}

PolarChartOptions.prototype.clearFlightNumbers = function() {
...
PolarChartOptions.prototype.setFlightNumbers = function(flightnums) {
    ...
    $('#'+me.uid+'_flight_num').on("selectmenuselect", function(event,ui) {
        ...
        if (ui.item.value !== 'placeholder') {
            ...
            airportsUS.getAirportFeature(destinationCode, function(feature){
                //console.log('panPopup to feature: ',feature);
                me.map.panPopup(feature, this.map);
            });
        }
    });
}
